...
**D3 — `funnel_state_planner` autoridad única (cableado nuevo).** Expone `plan(facts) -> FunnelState` con `profile_ready`, `missing_fields`, `conflicts`, `next_question`, `asked_field_keys`. **CORRECCIÓN de auditoría**: hoy `plan()` NO está invocado en vivo (`funnel_state_planner.py:29` solo lo lee un reader shadow) y su `CORE_FIELDS` **omite `candidate.name` y `candidate.age`** que los funnels vivos (`current_turn._next_funnel_question_or_none:535`, `_FUNNEL_STEPS:1463`) sí piden. Cablearlo como autoridad es **greenfield** y DEBE incorporar `candidate.name` y `candidate.age` a `CORE_FIELDS` (o justificar su exclusión sin perder esas preguntas) para no regresar el funnel. Los funnels vivos duplicados se eliminan DESPUÉS de cablear y verificar paridad. *Alternativa descartada*: adoptar `CORE_FIELDS` tal cual — deja de preguntar nombre/edad (regresión).
...
| Estado apto | `medical.apto_status` (`vigente|vencido|tramite`) | `document.apto_status` (singular, `profile_extractor.py:373`), `documents.general_status` | 3 variantes vivas → mapear todas al canónico; `apto_status`=estado, distinto de la vigencia |
| Vigencia apto | `medical.apto_expiration_text` | — | Fecha/plazo; el funnel LEE esta (`current_turn.py:569`), la nota lee `apto_status` (`chatwoot_note_sync.py:551`) → hoy divergen; `apto_status=vigente` requiere vigencia válida >3m |
...
Se **elimina todo mapeo implícito ambiguo**; cualquier lectura pasa por `funnel_state_planner`/adapter explícito. **CORRECCIÓN de auditoría**: `license.category` es lo que escriben/leen los paths VIVOS (`turn_extractor.py:49,330`, `memory_guard`/`turn_planner` que leen `license.type` son SHADOW). Fijar el canónico en `license.type` es un **rename en código 100% vivo** → el adapter de lectura (`category→type`) se implementa y verifica **ANTES** de tocar cualquier escritor, con la matriz de regresión en verde.

**D5 — Preguntas laterales + detector único (raíz del bug #3).** **CORRECCIÓN de auditoría**: el path vivo YA antepone la respuesta embebida (`knowledge_orchestrator.py:1972-1976`); el bug #3 NO es de composición sino de **DETECCIÓN**: `_resolve_embedded_question` (`:287-322`) descarta la pregunta si fallan sus dos gates (`_looks_like_question` heurístico + 2º clasificador que exige `requires_rag`), y el guard del worker usa OTRO detector (`signals.has_embedded_question`, `tasks_chatwoot.py:503`) — **en desacuerdo**. Decisión: **un único detector de pregunta embebida** (capability `embedded-question-detection`) consumido por guard y orquestador, que cubra el caso "compuesto sin `?` ni término de negocio conocido". Solo entonces la regla de pregunta lateral (responder + preservar pendiente + `next_question=None` + `should_continue_profile=True` + cierre suave opcional; NO anexar la siguiente pregunta) cierra el bug. *Alternativa descartada*: la D5 anterior (solo composición/autoridad) — no tocaba los detectores, no arreglaba #3.
